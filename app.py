...
# --- Charting Functions (styled for Google theme) ---
...
<style>
    ...
    /* Card titles used in your Python code */
    .card > div > h3 { /* Targets h3 directly inside a card's first div if that's Streamlit's structure */
    ...
    /* Section titles (h4) in your Python code */
     .card > div > h4 { 
        font-size: var(--md-sys-typescale-title-medium-font-size) !important; font-weight: 500 !important; 
        margin-top: 20px !important; margin-bottom: 12px !important; color: var(--md-sys-color-on-surface-variant) !important;
     }
     /* Sub-section titles (h5) in your Python code */
     .card > div > h5 { 
        font-size: 1.05em !important; color: var(--md-sys-color-on-surface) !important; margin-top: 18px !important; 
        margin-bottom: 10px !important; font-weight: 500 !important;
     }
     /* Smallest headers (h6) in your Python code, like "Individual Sales Progress" */
     .card > div > h6 { 
        font-size: 0.9em !important; color: var(--md-sys-color-on-surface-variant) !important; margin-top: 16px !important; 
        margin-bottom: 10px !important; font-weight: 500 !important; text-transform: uppercase; letter-spacing: 0.3px;
     }
    /* Form field labels (h6 with class) from your Python code */
    ...
    .welcome-text-sidebar { /* Your python uses this class */
        font-size: var(--md-sys-typescale-title-medium-font-size) !important; font-weight: 500 !important;
        color: var(--md-sys-color-on-surface) !important; margin-bottom: 4px !important;
    }
    [data-testid="stSidebar"] [data-testid="stImage"] > img { 
        border-radius: 50%; margin: 0px 0px 4px 0px; width: 36px !important; height: 36px !important;
        border: 1px solid var(--md-sys-color-outline-variant);
    }
    .user-position-sidebar { /* Your python uses this class */
...
if not st.session_state.auth["logged_in"]:
    ...
    uname = st.text_input("Username", key="login_uname_mainpage_key_v3_corrected") 
    pwd = st.text_input("Password", type="password", key="login_pwd_mainpage_key_v3_corrected") 
    if st.button("Login", key="login_button_mainpage_key_v3_corrected", use_container_width=True): 
...
with st.sidebar:
    ...
    selected_nav_display = st.radio(
        "Navigation", 
        nav_options_display, 
        key="sidebar_nav_main_radio_final_key_v2", 
...
if nav == "📆 Attendance":
    ...
    with col1:
        if st.button("Check In", key="check_in_btn_main_content_final_page_v2", use_container_width=True): 
            process_general_attendance("Check-In")
    with col2:
        if st.button("Check Out", key="check_out_btn_main_content_final_page_v2", use_container_width=True): 
    ...
    with st.form(key="activity_photo_form_content_final_v3"): 
        st.markdown("<h6 class='form-field-label-custom'>Capture and Describe Your Activity:</h6>", unsafe_allow_html=True) 
        activity_description = st.text_area("Brief description of activity/visit:", key="activity_desc_content_final_v3") 
        img_file_buffer_activity = st.camera_input("Take a picture of your activity/visit", key="activity_camera_input_content_final_v3") 
        submit_activity_photo = st.form_submit_button("⬆️ Upload Photo and Log Activity", key="activity_photo_form-Submit_v3") 
    ...
    with st.form(key="allowance_form_content_final_v3"): 
        st.markdown("<h6 class='form-field-label-custom'>Select Allowance Type:</h6>", unsafe_allow_html=True) 
        a_type = st.radio("", ["Travel", "Dinner", "Medical", "Internet", "Other"], key="allowance_type_radio_content_final_v3", horizontal=True, label_visibility='collapsed') 
        amount = st.number_input("Enter Amount (INR):", min_value=0.01, step=10.0, format="%.2f", key="allowance_amount_content_final_v3") 
        reason = st.text_area("Reason for Allowance:", key="allowance_reason_content_final_v3", placeholder="Please provide a clear justification...") 
        submitted_allowance = st.form_submit_button("Submit Allowance Request", use_container_width=True, key="submit_allowance_btn_content_final_v3") 
    ...
    if current_user["role"] == "admin":
        ...
        admin_action = st.radio("Action:", ["View Team Progress", f"Set/Edit Goal for {TARGET_GOAL_YEAR}"], key="admin_goal_action_radio_content_final_v3", horizontal=True) 
        if admin_action == "View Team Progress":
            ...
            if not employee_options: st.warning("No employees available.");
                ...
                selected_emp = st.radio("Select Employee:", employee_options, key="goal_emp_radio_admin_set_content_final_v3", horizontal=True) 
                quarter_options = [f"{TARGET_GOAL_YEAR}-Q{i}" for i in range(1,5)]; selected_period = st.radio("Goal Period:", quarter_options, key="goal_period_radio_admin_set_content_final_v3", horizontal=True) 
                ...
                with st.form(key=f"set_sales_goal_form_admin_content_final_v3_{selected_emp}_{selected_period}"): 
                    new_desc=st.text_area("Goal Description",value=g_desc,key=f"desc_admin_sales_content_final_v3_{selected_emp}_{selected_period}") 
                    new_target=st.number_input("Target Sales (INR)",value=g_target,min_value=0.0,step=1000.0,format="%.2f",key=f"target_admin_sales_content_final_v3_{selected_emp}_{selected_period}") 
                    new_achieved=st.number_input("Achieved Sales (INR)",value=g_achieved,min_value=0.0,step=100.0,format="%.2f",key=f"achieved_admin_sales_content_final_v3_{selected_emp}_{selected_period}") 
                    new_status=st.radio("Status:",status_options,index=status_options.index(g_status),horizontal=True,key=f"status_admin_sales_content_final_v3_{selected_emp}_{selected_period}") 
