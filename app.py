...
def render_goal_chart(df: pd.DataFrame, chart_title: str): # Retaining original colors from your functional code
    if df.empty:
    ...
    fig = px.bar(df_melted, x="MonthYear", y="Amount", color="Metric", barmode="group",
                 ...
                 title=chart_title,
                 color_discrete_map={'TargetAmount': '#3498db', 'AchievedAmount': '#2ecc71'}) # Original Colors
...
def create_donut_chart(progress_percentage, chart_title="Progress", achieved_color='#2ecc71', remaining_color='#f0f0f0', center_text_color=None): # Retaining original colors
    ...
    text_color_to_use = center_text_color if center_text_color else (achieved_color if actual_progress_display > 0 else '#4A4A4A') # Original text color logic
    ax.text(0, 0, f"{actual_progress_display:.0f}%", ha='center', va='center', fontsize=11, fontweight='bold', color=text_color_to_use, fontfamily='Roboto')
...
def create_team_progress_bar_chart(summary_df, title="Team Progress", target_col="Target", achieved_col="Achieved", user_col="Employee", target_color='#3498db', achieved_color='#2ecc71'): # Retaining original colors
    ...
    rects1 = ax.bar(x - width/2, target_amounts, width, label='Target', color=target_color, alpha=0.8) # Original alpha
    rects2 = ax.bar(x + width/2, achieved_amounts, width, label='Achieved', color=achieved_color, alpha=0.8) # Original alpha
    
    ax.set_ylabel('Amount (INR)', fontsize=10, color='var(--md-sys-color-on-surface-variant)', fontfamily='Roboto'); 
    ax.set_title(title, fontsize=13, fontweight='bold', pad=15, color='var(--md-sys-color-on-surface)', fontfamily='Roboto') # fontweight bold from original
    ax.set_xticks(x); 
    ...
    ax.yaxis.grid(True, linestyle='--', alpha=0.7, color='var(--md-sys-color-outline-variant)') # Original grid style
        ...
        for rect in rects:
            ...
            if height > 0: 
                ax.annotate(f'{height:,.0f}', xy=(rect.get_x() + rect.get_width() / 2, height), 
                            ...
                            fontsize=7, color=color_text, fontfamily='Roboto', fontweight='normal') # Original fontweight normal
    autolabel(rects1, '#333') # Original color
    autolabel(rects2, '#333') # Original color
...
<style>
    ...
    /* Typography from your original CSS if preferred, or use MD3 scales */
    h1, h2, h3, h4, h5, h6 { color: var(--md-sys-color-on-surface); font-weight: 500; }
     /* Card titles from your Python code */
    .card > div > h3 { /* Targets h3 directly inside a card's first div */
        margin-top: 0 !important; padding-bottom: 12px !important; margin-bottom: 18px !important;
        font-size: 1.75em !important; /* From original */ font-weight: 600 !important; /* From original */
        border-bottom: 2px solid var(--md-sys-color-outline-variant) !important; /* Adjusted border */
        color: var(--md-sys-color-primary) !important; /* Original primary color for titles */
    }
     .card > div > h4 { 
        font-size: 1.4em !important; font-weight: 600 !important; 
        margin-top: 20px !important; margin-bottom: 12px !important; 
        color: var(--md-sys-color-secondary) !important; /* Using MD secondary */
        padding-bottom: 8px !important; border-bottom: 1px solid var(--md-sys-color-outline-variant) !important;
     }
     .card > div > h5 { 
        font-size: 1.15em !important; color: var(--md-sys-color-on-surface) !important; margin-top: 18px !important; 
        margin-bottom: 10px !important; font-weight: 600 !important;
     }
     .card > div > h6 { 
        font-size: 0.95em !important; color: var(--md-sys-color-on-surface-variant) !important; margin-top: 16px !important; 
        margin-bottom: 10px !important; font-weight: 500 !important;
     }
    h6.form-field-label-custom { /* Your Python's h6 for form field labels */
        font-size: 1em !important; font-weight: 500 !important;
        color: var(--md-sys-color-on-surface-variant) !important; margin-top: 20px !important;
        margin-bottom: 10px !important; text-transform: uppercase !important; letter-spacing: 0.5px !important;
    }
    ...
    [data-testid="stSidebar"] .sidebar-content { 
        padding: 12px 0px !important; overflow-y: auto; flex-grow: 1;
    }
    
    /* Sidebar Header (welcome text and user info) */
    .welcome-text { /* This class is used in your Python for the welcome message */
        font-size: 1.4em !important; font-weight: 600 !important; margin-bottom: 15px !important; 
    ...
    [data-testid="stSidebar"] [data-testid="stImage"] > img { 
        ...
        border: 2px solid var(--md-sys-color-primary-container); /* Accent border for avatar */
    }
    [data-testid="stSidebar"] p[style*="text-align:center"] { /* User position - from your Python */
        font-size: 0.9em !important; color: var(--md-sys-color-on-surface-variant) !important;
        margin-left: 20px !important; margin-top: -4px !important; margin-bottom: 16px !important;
        text-align: left !important;
    }
    [data-testid="stSidebar"] hr { 
        margin: 12px 20px !important; border-color: var(--md-sys-color-outline-variant) !important; 
    }
    ...
    /* Sidebar Logout Button (using original button styling as a base) */
    ...
    [data-testid="stSidebar"] .stButton button[id*="logout_button_sidebar"] {
        background-color: var(--md-sys-color-error) !important; /* Original danger color */
        border: 1px solid var(--md-sys-color-error) !important; 
        color: var(--md-sys-color-on-error) !important; 
        font-weight: 500 !important;
        border-radius: var(--md-sys-shape-corner-full) !important;
        padding: 8px 16px !important;
        margin: 0 16px !important; /* Give it some horizontal margin */
        width: calc(100% - 32px) !important;
    }
    [data-testid="stSidebar"] .stButton button[id*="logout_button_sidebar"]:hover {
        background-color: color-mix(in srgb, var(--md-sys-color-error) 85%, black) !important; 
        border-color: color-mix(in srgb, var(--md-sys-color-error) 85%, black) !important;
    }
...
if not st.session_state.auth["logged_in"]:
    ...
    uname = st.text_input("Username", key="login_uname_mainpage_key_v3_corrected_final") 
    pwd = st.text_input("Password", type="password", key="login_pwd_mainpage_key_v3_corrected_final") 
    if st.button("Login", key="login_button_mainpage_key_v3_corrected_final", use_container_width=True): 
...
with st.sidebar:
    ...
    selected_nav_display = st.radio(
        "Navigation", 
        nav_options_display, 
        key="sidebar_nav_main_radio_final_key_v3", 
...
# --- Main Content ---
if nav == "📆 Attendance": # Logic key from nav_logic_map
    ...
    with col1:
        if st.button("Check In", key="check_in_btn_main_content_final_page_v3", use_container_width=True): 
            process_general_attendance("Check-In")
    with col2:
        if st.button("Check Out", key="check_out_btn_main_content_final_page_v3", use_container_width=True): 
    ...
    with st.form(key="activity_photo_form_content_final_v3_corrected"): 
        st.markdown("<h6 class='form-field-label-custom'>Capture and Describe Your Activity:</h6>", unsafe_allow_html=True) 
        activity_description = st.text_area("Brief description of activity/visit:", key="activity_desc_content_final_v3_corrected") 
        img_file_buffer_activity = st.camera_input("Take a picture of your activity/visit", key="activity_camera_input_content_final_v3_corrected") 
        submit_activity_photo = st.form_submit_button("⬆️ Upload Photo and Log Activity", key="activity_photo_form-Submit_v3_corrected") 

    if submit_activity_photo:
        global activity_log_df # Corrected Placement
    ...
    with st.form(key="allowance_form_content_final_v3_corrected"): 
        st.markdown("<h6 class='form-field-label-custom'>Select Allowance Type:</h6>", unsafe_allow_html=True) 
        a_type = st.radio("", ["Travel", "Dinner", "Medical", "Internet", "Other"], key="allowance_type_radio_content_final_v3_corrected", horizontal=True, label_visibility='collapsed') 
        amount = st.number_input("Enter Amount (INR):", min_value=0.01, step=10.0, format="%.2f", key="allowance_amount_content_final_v3_corrected") 
        reason = st.text_area("Reason for Allowance:", key="allowance_reason_content_final_v3_corrected", placeholder="Please provide a clear justification...") 
        submitted_allowance = st.form_submit_button("Submit Allowance Request", use_container_width=True, key="submit_allowance_btn_content_final_v3_corrected") 

        if submitted_allowance:
            global allowance_df # Corrected Placement
    ...
    if current_user["role"] == "admin":
        ...
        admin_action = st.radio("Action:", ["View Team Progress", f"Set/Edit Goal for {TARGET_GOAL_YEAR}"], key="admin_goal_action_radio_content_final_v3_corrected", horizontal=True) 
        if admin_action == "View Team Progress":
            ...
            if not employee_options: st.warning("No employees available.");
                ...
                selected_emp = st.radio("Select Employee:", employee_options, key="goal_emp_radio_admin_set_content_final_v3_corrected", horizontal=True) 
                quarter_options = [f"{TARGET_GOAL_YEAR}-Q{i}" for i in range(1,5)]; selected_period = st.radio("Goal Period:", quarter_options, key="goal_period_radio_admin_set_content_final_v3_corrected", horizontal=True) 
                ...
                with st.form(key=f"set_sales_goal_form_admin_content_final_v3_corrected_{selected_emp}_{selected_period}"): 
                    new_desc=st.text_area("Goal Description",value=g_desc,key=f"desc_admin_sales_content_final_v3_corrected_{selected_emp}_{selected_period}") 
                    new_target=st.number_input("Target Sales (INR)",value=g_target,min_value=0.0,step=1000.0,format="%.2f",key=f"target_admin_sales_content_final_v3_corrected_{selected_emp}_{selected_period}") 
                    new_achieved=st.number_input("Achieved Sales (INR)",value=g_achieved,min_value=0.0,step=100.0,format="%.2f",key=f"achieved_admin_sales_content_final_v3_corrected_{selected_emp}_{selected_period}") 
                    new_status=st.radio("Status:",status_options,index=status_options.index(g_status),horizontal=True,key=f"status_admin_sales_content_final_v3_corrected_{selected_emp}_{selected_period}") 
                    submitted=st.form_submit_button("Save Goal", key="set_goal_form-Submit_v3_corrected") 

                    if submitted:
                        global goals_df # Corrected Placement
        ...
        if not current_g_df.empty:
            ...
            st.markdown("<hr>", unsafe_allow_html=True)
            with st.form(key=f"update_sales_achievement_form_employee_content_final_v3_{current_user['username']}_{current_quarter_for_display}"): 
                new_val=st.number_input("Update Achieved Amount (INR):",value=achieved_amt,min_value=0.0,step=100.0,format="%.2f", key=f"employee_sales_ach_update_content_final_v3_{current_user['username']}_{current_quarter_for_display}") 
                submitted_ach=st.form_submit_button("Update Achievement", key="update_achievement-Submit_v2") 
                if submitted_ach:
                    global goals_df # Corrected Placement
    ...
    if current_user["role"] == "admin":
        ...
        admin_action_payment = st.radio("Action:", ["View Team Progress", f"Set/Edit Collection Target for {TARGET_YEAR_PAYMENT}"], key="admin_payment_action_radio_content_final_v3", horizontal=True) 
        if admin_action_payment == "View Team Progress":
            ...
            if not employees_for_payment_goal: st.warning("No employees available.")
                ...
                selected_emp_payment=st.radio("Select Employee:",employees_for_payment_goal,key="payment_emp_radio_admin_set_content_final_v3", horizontal=True) 
                quarters_payment=[f"{TARGET_YEAR_PAYMENT}-Q{i}" for i in range(1,5)]; selected_period_payment=st.radio("Quarter:",quarters_payment,key="payment_period_radio_admin_set_content_final_v3", horizontal=True) 
                ...
                with st.form(f"form_payment_admin_content_final_v3_{selected_emp_payment}_{selected_period_payment}"): 
                    new_desc_payment=st.text_input("Collection Goal Description",value=desc_payment,key=f"desc_admin_payment_content_final_v3_{selected_emp_payment}_{selected_period_payment}") 
                    new_tgt_payment=st.number_input("Target Collection (INR)",value=tgt_payment_val,min_value=0.0,step=1000.0,key=f"target_admin_payment_content_final_v3_{selected_emp_payment}_{selected_period_payment}") 
                    new_ach_payment=st.number_input("Collected Amount (INR)",value=ach_payment_val,min_value=0.0,step=500.0,key=f"achieved_admin_payment_content_final_v3_{selected_emp_payment}_{selected_period_payment}") 
                    new_status_payment=st.selectbox("Status",status_options_payment,index=status_options_payment.index(stat_payment),key=f"status_admin_payment_content_final_v3_{selected_emp_payment}_{selected_period_payment}") 
                    submitted_payment=st.form_submit_button("Save Goal", key="form_payment-Submit_v2") 

                    if submitted_payment:
                        global payment_goals_df # Corrected Placement
        ...
        if not current_payment_goal_period_df.empty:
            ...
            st.markdown("<hr>", unsafe_allow_html=True)
            with st.form(key=f"update_payment_collection_form_employee_content_final_v3_{current_user['username']}_{current_quarter_display_payment}"): 
                new_ach_val_payment=st.number_input("Update Collected Amount (INR):",value=ach_pay,min_value=0.0,step=500.0, key=f"employee_payment_ach_update_content_final_v3_{current_user['username']}_{current_quarter_display_payment}") 
                submit_collection_update=st.form_submit_button("Update Collection", key="update_collection-Submit_v2") 
    ...
    if current_user["role"] == "admin":
        ...
        selected_employee_log = st.selectbox("Select Employee:", employee_name_list, key="log_employee_select_admin_content_final_page_v2", index=0 if employee_name_list else None) 
