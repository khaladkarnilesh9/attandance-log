...
# Load dataframes - these are now global and will be modified by page logic
...
<style>
    ...
    .user-profile-img-display { /* This class is for the <img> tag itself */
    ...
    .button-column-container .stButton button { width: 100%; } /* For Check In/Out buttons */
...
# --- PLOTTING FUNCTIONS ---
def render_goal_chart(df: pd.DataFrame, chart_title: str): # Copied from your original
...
def create_donut_chart(progress_percentage, chart_title="Progress", achieved_color='#20BEFF', remaining_color='#E0E0E0', center_text_color='#333333'): # Copied
...
def create_team_progress_bar_chart(summary_df, title="Team Progress", target_col="Target", achieved_col="Achieved", user_col="Employee"): # Copied
...
if not st.session_state.auth["logged_in"]:
    ...
    uname = st.text_input("Username", key="login_uname_app_final") # Ensure unique keys
    pwd = st.text_input("Password", type="password", key="login_pwd_app_final")
    if st.button("Login", key="login_button_app_final", type="primary", use_container_width=True):
...
current_user = st.session_state.auth # current_user is now set after successful login
...
with st.sidebar:
    ...
    current_username_sb = current_user.get('username', 'Guest') # Use .get for safety
    ...
    selected = option_menu(
        ...
        key='main_app_option_menu_final',
        styles={
    ...
    if st.button("🚪 Logout", key="logout_app_button_key_final", use_container_width=True):
...
if selected == "Attendance":
    ...
    st.markdown("---")
    # Ensure current_user is available here
    ...
    with col1_att:
        if st.button("✅ Check In", key="check_in_btn_att", use_container_width=True, type="primary"): process_general_attendance("Check-In")
    with col2_att:
        if st.button("🚪 Check Out", key="check_out_btn_att", use_container_width=True, type="primary"): process_general_attendance("Check-Out")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    with st.form(key="activity_photo_form_upload_main"):
        st.markdown("<h6>Capture and Describe Your Activity:</h6>", unsafe_allow_html=True)
        activity_description = st.text_area("Brief description:", key="activity_desc_upload_main")
        img_file_buffer_activity = st.camera_input("Take a picture:", key="activity_camera_upload_main")
    ...
    a_type = st.radio("Type:", ["Travel", "Dinner", "Medical", "Internet", "Other"], key="allowance_type_radio_main_allow", horizontal=True)
    amount = st.number_input("Amount (INR):", min_value=0.01, step=10.0, format="%.2f", key="allowance_amount_main_allow")
    reason = st.text_area("Reason:", key="allowance_reason_main_allow", placeholder="Please provide a clear justification...")
    if st.button("Submit Allowance Request", key="submit_allowance_btn_main_allow", use_container_width=True, type="primary"):
    ...
    if current_user["role"] == "admin": # current_user is from st.session_state.auth
        st.markdown("<h4>Admin: Manage & Track Employee Goals</h4>", unsafe_allow_html=True)
        admin_action = st.radio("Action:", ["View Team Progress", f"Set/Edit Goal for {TARGET_GOAL_YEAR}"], key="admin_goal_action_radio_gt", horizontal=True)
        if admin_action == "View Team Progress":
                ...
                if not summary_df_sales.empty:
                    ...
                    if num_cols_display_gt > 0:
                        ...
                        for idx_gt, (index_gt, row_gt) in enumerate(summary_df_sales.iterrows()):
                            ...
                            donut_fig_gt = create_donut_chart(progress_percent_gt, achieved_color='#28a745') # Kaggle-like green
            ...
            if not employee_options_gt: st.warning("No employees available.")
                ...
                selected_emp_gt = st.radio("Select Employee:", employee_options_gt, key="goal_emp_radio_admin_gt", horizontal=True)
                quarter_options_gt = [f"{TARGET_GOAL_YEAR}-Q{i}" for i in range(1,5)]; selected_period_gt = st.radio("Goal Period:", quarter_options_gt, key="goal_period_radio_admin_gt", horizontal=True)
                existing_g_gt = goals_df[(goals_df["Username"] == selected_emp_gt) & (goals_df["MonthYear"] == selected_period_gt)]
                ...
                with st.form(key=f"set_goal_form_{selected_emp_gt}_{selected_period_gt}_admin_final"):
                    new_desc_gt=st.text_area("Goal Description",value=g_desc_gt,key=f"desc_gt_final")
                    new_target_gt=st.number_input("Target Sales (INR)",value=float(g_target_gt),min_value=0.0,step=1000.0,format="%.2f",key=f"target_gt_final")
                    new_achieved_gt=st.number_input("Achieved Sales (INR)",value=float(g_achieved_gt),min_value=0.0,step=100.0,format="%.2f",key=f"achieved_gt_final")
                    new_status_gt=st.radio("Status:",status_options,index=status_options.index(g_status_gt),horizontal=True,key=f"status_gt_final")
                ...
                if submitted_gt:
                    if not new_desc_gt.strip(): st.warning("Description required.")
                        ...
                        global goals_df # To modify the global dataframe
                        idx_to_update_gt = goals_df[(goals_df["Username"] == selected_emp_gt) & (goals_df["MonthYear"] == selected_period_gt)].index
                        if not idx_to_update_gt.empty:
                            goals_df.loc[idx_to_update_gt, ["GoalDescription", "TargetAmount", "AchievedAmount", "Status"]] = [new_desc_gt, new_target_gt, new_achieved_gt, new_status_gt]
                            msg_verb_gt="updated"
                        else:
                            ...
                            goals_df=pd.concat([goals_df,new_row_df_gt],ignore_index=True)
                            msg_verb_gt="set"
                        try:
        ...
        if not current_g_df_e_gt.empty:
            ...
            st.markdown("---")
            with st.form(key=f"update_achievement_{current_user['username']}_{current_quarter_for_display}_final_gt"):
            ...
            if submitted_ach_e_gt:
                global goals_df # Ensure modification of global df
                idx_update_e_gt = goals_df[(goals_df["Username"] == current_user["username"]) & (goals_df["MonthYear"] == current_quarter_for_display)].index
                if not idx_update_e_gt.empty:
                    goals_df.loc[idx_update_e_gt[0],"AchievedAmount"]=new_val_e_gt
                    goals_df.loc[idx_update_e_gt[0],"Status"] = "Achieved" if new_val_e_gt >= target_amt_e_gt and target_amt_e_gt > 0 else "In Progress"
                    try:
                        goals_df.to_csv(GOALS_FILE,index=False)
    ...
    if current_user["role"] == "admin":
        ...
        admin_action_payment = st.radio("Action:", ["View Team Progress", f"Set/Edit Collection Target for {TARGET_YEAR_PAYMENT}"], key="admin_payment_action_admin_set_pct", horizontal=True)
        if admin_action_payment == "View Team Progress":
            ...
            if not employees_payment_list: st.info("No employees found.")
                ...
                for emp_pay_name in employees_payment_list:
                    ...
                    if not record_payment.empty:
                        rec_payment=record_payment.iloc[0]; target_p=float(pd.to_numeric(rec_payment.get("TargetAmount"),errors='coerce') or 0.0)
                        achieved_p=float(pd.to_numeric(rec_payment.get("AchievedAmount"),errors='coerce') or 0.0); status_p=rec_payment.get("Status","N/A")
                ...
                if not summary_df_payment.empty:
                    ...
                    num_cols_payment_display = min(3, len(summary_df_payment))
                    if num_cols_payment_display > 0:
                        cols_payment = st.columns(num_cols_payment_display)
                        for idx_p, (index_p,row_p) in enumerate(summary_df_payment.iterrows()):
                            progress_percent_p=(row_p['Achieved']/row_p['Target']*100) if row_p['Target'] > 0 else 0
                            donut_fig_p=create_donut_chart(progress_percent_p,achieved_color='#2070c0') # Blue for payment
                            with cols_payment[idx_p % num_cols_payment_display]:
            ...
            if not employees_for_payment_goal: st.warning("No employees available.")
                ...
                selected_emp_payment=st.radio("Select Employee:",employees_for_payment_goal,key="payment_emp_radio_admin_set_pct",horizontal=True)
                quarters_payment=[f"{TARGET_YEAR_PAYMENT}-Q{i}" for i in range(1,5)]; selected_period_payment=st.radio("Quarter:",quarters_payment,key="payment_period_radio_admin_set_pct",horizontal=True)
                ...
                desc_payment,tgt_payment_val,ach_payment_val,stat_payment = "",0.0,0.0,status_options_payment[0]
                if not existing_payment_goal.empty:
                    g_payment=existing_payment_goal.iloc[0]; desc_payment=g_payment.get("GoalDescription",""); tgt_payment_val=g_payment.get("TargetAmount",0.0)
                    ach_payment_val=g_payment.get("AchievedAmount",0.0); stat_payment=g_payment.get("Status",status_options_payment[0])
                    st.info(f"Editing payment goal for {selected_emp_payment} - {selected_period_payment}")
                with st.form(f"form_payment_{selected_emp_payment}_{selected_period_payment}_admin_pct"):
                    new_desc_payment=st.text_input("Collection Goal Description",value=desc_payment,key=f"desc_pay_pct_admin")
                    new_tgt_payment=st.number_input("Target Collection (INR)",value=float(tgt_payment_val),min_value=0.0,step=1000.0,key=f"target_pay_pct_admin")
                    new_ach_payment=st.number_input("Collected Amount (INR)",value=float(ach_payment_val),min_value=0.0,step=500.0,key=f"achieved_pay_pct_admin")
                    new_status_payment=st.selectbox("Status",status_options_payment,index=status_options_payment.index(stat_payment),key=f"status_pay_pct_admin")
                ...
                if submitted_payment:
                    if not new_desc_payment.strip(): st.warning("Description required.")
                        ...
                        global payment_goals_df # To modify global dataframe
                        idx_to_update_pct = payment_goals_df[(payment_goals_df["Username"]==selected_emp_payment)&(payment_goals_df["MonthYear"]==selected_period_payment)].index
                        if not idx_to_update_pct.empty:
                            payment_goals_df.loc[idx_to_update_pct[0]]=[selected_emp_payment,selected_period_payment,new_desc_payment,new_tgt_payment,new_ach_payment,new_status_payment]
                            msg_payment="updated"
                        else:
                            new_row_df_p=pd.DataFrame([{"Username":selected_emp_payment,"MonthYear":selected_period_payment,"GoalDescription":new_desc_payment,"TargetAmount":new_tgt_payment,"AchievedAmount":new_ach_payment,"Status":new_status_payment}], columns=PAYMENT_GOALS_COLUMNS)
                            payment_goals_df=pd.concat([payment_goals_df,new_row_df_p],ignore_index=True)
                            msg_payment="set"
                        try:
        ...
        user_goals_payment_e = payment_goals_df[payment_goals_df["Username"]==current_user["username"]].copy()
        user_goals_payment_e[["TargetAmount","AchievedAmount"]] = user_goals_payment_e[["TargetAmount","AchievedAmount"]].apply(pd.to_numeric,errors="coerce").fillna(0.0)
        current_payment_goal_e_df = user_goals_payment_e[user_goals_payment_e["MonthYear"]==current_quarter_display_payment]
        st.markdown(f"<h5>Current Quarter: {current_quarter_display_payment}</h5>", unsafe_allow_html=True)
        if not current_payment_goal_e_df.empty:
            g_pay_e=current_payment_goal_e_df.iloc[0]; tgt_pay_e=g_pay_e["TargetAmount"]; ach_pay_e=g_pay_e["AchievedAmount"]
            st.markdown(f"**Goal:** {g_pay_e.get('GoalDescription','')}")
            col_metrics_pay_e,col_chart_pay_e=st.columns([0.63,0.37])
            with col_metrics_pay_e:
                sub_col1_pay_e,sub_col2_pay_e=st.columns(2); sub_col1_pay_e.metric("Target",f"₹{tgt_pay_e:,.0f}"); sub_col2_pay_e.metric("Collected",f"₹{ach_pay_e:,.0f}")
                st.metric("Status",g_pay_e.get("Status","In Progress"),label_visibility="labeled")
            with col_chart_pay_e:
                progress_percent_pay_e=(ach_pay_e/tgt_pay_e*100) if tgt_pay_e > 0 else 0.0
                st.markdown(f"<h6 style='text-align:center;margin-bottom:0px;margin-top:-15px;'>Collection Progress</h6>",unsafe_allow_html=True)
                donut_fig_payment_e=create_donut_chart(progress_percent_pay_e,"Collection Progress",achieved_color='#2070c0'); st.pyplot(donut_fig_payment_e,use_container_width=True)
            st.markdown("---")
            with st.form(key=f"update_collection_{current_user['username']}_{current_quarter_display_payment}_final_pct"):
                new_ach_val_payment_e=st.number_input("Update Collected Amount (INR):",value=ach_pay_e,min_value=0.0,step=500.0)
                submit_collection_update_e=st.form_submit_button("Update Collection")
            if submit_collection_update_e:
                global payment_goals_df # To modify global dataframe
                idx_pay_e=payment_goals_df[(payment_goals_df["Username"]==current_user["username"])&(payment_goals_df["MonthYear"]==current_quarter_display_payment)].index
                if not idx_pay_e.empty:
                    payment_goals_df.loc[idx_pay_e[0],"AchievedAmount"]=new_ach_val_payment_e
                    payment_goals_df.loc[idx_pay_e[0],"Status"]="Achieved" if new_ach_val_payment_e >= tgt_pay_e and tgt_pay_e > 0 else "In Progress"
                    try:
                        payment_goals_df.to_csv(PAYMENT_GOALS_FILE,index=False)
        ...
        past_payment_goals_e = user_goals_payment_e[user_goals_payment_e["MonthYear"]!=current_quarter_display_payment]
        if not past_payment_goals_e.empty: render_goal_chart(past_payment_goals_e,"Past Collection Performance")
    ...
    def display_activity_logs_with_photos(df_logs, user_name_for_header): # Copied from original
    ...
    def display_attendance_logs(df_logs, user_name_for_header): # Copied from original
    ...
    if current_user["role"] == "admin":
        ...
        employee_name_list = [uname for uname, udata in USERS.items() if udata["role"] == "employee"] # Filtered
        if not employee_name_list: st.info("No employees to display logs for.")
        else:
            ...
            if selected_employee_log:
                ...
                else: st.info("No allowance records found") # Changed to info
    ...
    store_name = st.selectbox("Select Store", sorted(stores_df["StoreName"].dropna().astype(str).unique()), key="co_store")
    # selected_store_details = stores_df[stores_df["StoreName"] == store_name].iloc[0] # Not used directly

    product_name = st.selectbox("Select Product", sorted(products_df["Product Name"].dropna().astype(str).unique()), key="co_product")
    product_sizes_df = products_df[products_df["Product Name"] == product_name]
    
    size_options = sorted(product_sizes_df["Size"].dropna().astype(str).unique()) if not product_sizes_df.empty else []
    if not size_options: st.info(f"No sizes available for {product_name}.")
    
    size = st.selectbox("Select Size", size_options, key="co_size", disabled=not size_options)
    quantity = st.number_input("Enter Quantity", min_value=1, value=1, key="co_quantity", disabled=not size_options)

    if size and st.button("Add to Order", key="co_add_btn", type="primary", disabled=not size_options):
        selected_product_row = product_sizes_df[product_sizes_df["Size"] == size]
        if not selected_product_row.empty:
            selected_product = selected_product_row.iloc[0]
            unit_price = pd.to_numeric(selected_product.get("Price"), errors='coerce')
            if pd.notna(unit_price):
                item = {
                    "Store": store_name, "Product": product_name, "Size": size,
                    "Quantity": quantity, "Unit Price": unit_price, "Total": unit_price * quantity
                }
                st.session_state.order_items.append(item) # Assumes order_items is initialized
                st.success(f"Added to order: {quantity} x {product_name} ({size})")
            else: st.warning(f"Price not available for {product_name} ({size}).")
        else: st.warning("Selected product size details not found.")

    if st.session_state.order_items:
        st.subheader("🧾 Order Summary")
        order_summary_df = pd.DataFrame(st.session_state.order_items)
        order_summary_df["Unit Price"] = pd.to_numeric(order_summary_df["Unit Price"], errors='coerce').fillna(0)
        order_summary_df["Total"] = pd.to_numeric(order_summary_df["Total"], errors='coerce').fillna(0)
        
        display_df = order_summary_df.copy()
        for col_currency in ["Unit Price", "Total"]: display_df[col_currency] = display_df[col_currency].apply(lambda x: f"₹{x:,.2f}")
        st.dataframe(display_df[["Store", "Product", "Size", "Quantity", "Unit Price", "Total"]], use_container_width=True, hide_index=True)
        grand_total = order_summary_df['Total'].sum()
        st.markdown(f"<h4 style='text-align: right; margin-top: 1rem;'>Grand Total: ₹{grand_total:,.2f}</h4>", unsafe_allow_html=True)
        if st.button("Clear Order", key="co_clear_btn"):
