...
if nav == "📆 Attendance":
    ...
    if current_user["role"] == "admin":
        ...
        if not past_g.empty:
            st.dataframe(past_g[["MonthYear", "GoalDescription", "TargetAmount", "AchievedAmount", "Status"]], hide_index=True, use_container_width=True,
        ...
        else: st.info("No past goal records found.")
    st.markdown('</div>', unsafe_allow_html=True)

elif nav == "📊 View Logs": # Restored detailed view
    st.markdown('<div class="card">', unsafe_allow_html=True)
    if current_user["role"] == "admin":
        ...
        if not employee_names: st.info("No employees found or no employee data to display.")
            ...
            for emp_name in employee_names:
                ...
                profile_col1, profile_col2 = st.columns([1, 4]) # Adjusted ratio
                with profile_col1:
                    if user_info.get("profile_photo") and os.path.exists(user_info.get("profile_photo")):
                        st.image(user_info.get("profile_photo"), width=80) # Slightly smaller
                with profile_col2:
                ...
                if not emp_attendance.empty:
                    ...
                    st.markdown("<h6 class='allowance-summary-header'>🗺️ Attendance Locations Map:</h6>", unsafe_allow_html=True) # CSS class for h6
                ...
                if not emp_allowances.empty:
                    ...
                    st.markdown("<h6 class='allowance-summary-header'>📅 Monthly Allowance Summary:</h6>", unsafe_allow_html=True) # CSS class
                    ...
                    st.markdown("<h6 class='allowance-summary-header'>📋 Detailed Allowance Requests:</h6>", unsafe_allow_html=True) # CSS class
                ...
                if emp_name != employee_names[-1]: st.markdown("<hr style='margin-top: 25px; margin-bottom:10px;'>", unsafe_allow_html=True) # Clearer separator
        ...
        final_display_df, my_att_proc = pd.DataFrame(), pd.DataFrame()
        if not my_att_raw.empty:
            ...
            if not my_att_proc.empty:
                ...
                # ... (Your detailed check-in/check-out processing logic from before) ...
                # This is a complex block, I'll keep your previous logic here:
    ...
    # Only check-outs available, create empty check-in columns
    ...
    combined_df = pd.DataFrame(columns=list(dict.fromkeys(first_check_in_cols + last_check_out_cols)))
